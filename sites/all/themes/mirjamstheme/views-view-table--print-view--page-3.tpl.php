	...
	foreach ($row as $field => $content):
		...
		switch ($fields[$field]) {
			...
			case "field-salescomp-nid":
					$jeff['left'][] = $content;
					$jeff['left_title'][] = "Sales Co:";
					$right++;
				break;
			case "field-mirjamread-value":
					$jeff['right'][] = $content;
					$jeff['right_title'][] = "Read By:";
					$right++;
				break;
...
for ($k=0; $k<= max($left, $right); $k++) 
{
	...
	print "</table><table class='print_middle_box'><colgroup><col width='15%'><col width='85%'></colgroup>";
	...
	print "<div class='coverage'>Coverage:<br/>".($coveragetext)."</div>";
	
$cvg = '<EMBED CLASS="xyz" SRC="http://www.orange-ent.net/htdocs/sites/default/files/'.$coverage.'"#view=FullScreenEmbed,top&pagemode=none&scrollbar=1&statusbar=0&messages=1&navpanes=1" "BORDER=2px HEIGHT=500 WIDTH=100% ALT="">';
